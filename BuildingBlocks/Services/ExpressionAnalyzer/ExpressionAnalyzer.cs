...
namespace BuildingBlocks.Services.ExpressionAnalyzer
{
    public static class ExpressionAnalyzer
    {
        private static readonly Router _router = new Router();

        public static ValidationResponse Analyze(string inputExpression)
        {
            ...
            if (string.IsNullOrWhiteSpace(inputExpression))
            {
                ...
                return new ValidationResponse { Errors = errors, Tokens = tokens };
            }
            ...
            for (int i = 0; i < inputExpression.Length; i++)
            {
                ...
                if (currentToken.Length > 0 && isTokenBoundary)
                {
                    ...
                    {
                        TokenType tokenType = currState == State.FUNCTION_OPENING_BRACKET ? TokenType.FN : TokenType.VAR;
                        tokens.Add(new Token { TokenType = tokenType, Value = currentToken.ToString() });
                    }
                    else
                    {
        ...
        private static void FinalizeToken(List<Token> tokens, string tokenValue, State tokenState)
        {
            ...
            switch (tokenState)
            {
                ...
                case State.FUNCTION_OPENING_BRACKET:
                    tokenType = TokenType.OB;
                    break;
                case State.CLOSING_BRACKET:
                    tokenType = TokenType.CB;
                    break;
            }
            tokens.Add(new Token { TokenType = tokenType, Value = tokenValue });
        }
